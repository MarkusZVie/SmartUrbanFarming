...
@Rule(name = "LightSensorLongTermTooLow", description = "Create Message Light Longterm depending")
public class LightSensorLongTermTooLow {
	
	private final String ruleName = "LightSensorLongTermTooLow";
    ...
    public void then() throws Exception {
		...
		for(ArrayList<String> cropAttributes: cropInformationList) {
			...
			if(cropAttributes.get(1).equals("low")) {
				message = "Die Pflanze: '"+cropAttributes.get(0)+"' hat die Licht Verträglichkeit von '"+cropAttributes.get(1)+"' hatte dieses Monat jedoch "+(Double.parseDouble(cropAttributes.get(3)+"")-lowMinimumValue)*100+" % weniger licht als minderst erforderlich (insgesamt "+((Double.parseDouble(cropAttributes.get(3)+""))*100)*(-1)+" % weniger als erwartet) und einen eine Durchschnitts Belichtung von "+cropAttributes.get(2)+".";
			}
			if(cropAttributes.get(1).equals("medium")) {
				message = "Die Pflanze: '"+cropAttributes.get(0)+"' hat die Licht Verträglichkeit von '"+cropAttributes.get(1)+"' hatte dieses Monat jedoch "+(Double.parseDouble(cropAttributes.get(3)+"")-mediumMinimumValue)*100+" % weniger licht als minderst erforderlich (insgesamt "+((Double.parseDouble(cropAttributes.get(3)+""))*100)*(-1)+" % weniger als erwartet) und einen eine Durchschnitts Belichtung von "+cropAttributes.get(2)+".";
			}
			if(cropAttributes.get(1).equals("high")) {
				message = "Die Pflanze: '"+cropAttributes.get(0)+"' hat die Licht Verträglichkeit von '"+cropAttributes.get(1)+"' hatte dieses Monat jedoch "+(Double.parseDouble(cropAttributes.get(3)+"")-highMinimumValue)*100+" % weniger licht als minderst erforderlich (insgesamt "+((Double.parseDouble(cropAttributes.get(3)+""))*100)*(-1)+" % weniger als erwartet) und einen eine Durchschnitts Belichtung von "+cropAttributes.get(2)+".";
			}
