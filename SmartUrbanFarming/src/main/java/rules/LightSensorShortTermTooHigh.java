...
@Rule(name = "LightSensorShortTermTooHigh", description = "Create Message Light Longterm depending")
public class LightSensorShortTermTooHigh {
	
	private final String ruleName = "LightSensorShortTermTooHigh";
    ...
    public void then() throws Exception {
		...
		for(ArrayList<String> cropAttributes: cropInformationList) {
			...
			if(cropAttributes.get(1).equals("low")) {
				message = "Die Pflanze: '"+cropAttributes.get(0)+"' hat die Licht Verträglichkeit von '"+cropAttributes.get(1)+"' hatte heute jedoch "+(Double.parseDouble(cropAttributes.get(3)+"")-lowMaximumValue)*100+" % mehr licht als maximal zulässig (insgesamt "+((Double.parseDouble(cropAttributes.get(3)+""))*100)*(-1)+" % mehr als erwartet) und einen eine Durchschnitts Belichtung von "+cropAttributes.get(2)+".";
			}
			if(cropAttributes.get(1).equals("medium")) {
				message = "Die Pflanze: '"+cropAttributes.get(0)+"' hat die Licht Verträglichkeit von '"+cropAttributes.get(1)+"' hatte heute jedoch "+(Double.parseDouble(cropAttributes.get(3)+"")-mediumMaximumValue)*100+" % mehr licht als maximal zulässig  (insgesamt "+((Double.parseDouble(cropAttributes.get(3)+""))*100)*(-1)+" % mehr als erwartet) und einen eine Durchschnitts Belichtung von "+cropAttributes.get(2)+".";
			}
			if(cropAttributes.get(1).equals("high")) {
				message = "Die Pflanze: '"+cropAttributes.get(0)+"' hat die Licht Verträglichkeit von '"+cropAttributes.get(1)+"' hatte heute jedoch "+(Double.parseDouble(cropAttributes.get(3)+"")-highMaximumValue)*100+" % mehr licht als maximal zulässig  (insgesamt "+((Double.parseDouble(cropAttributes.get(3)+""))*100)*(-1)+" % mehr als erwartet) und einen eine Durchschnitts Belichtung von "+cropAttributes.get(2)+".";
			}
